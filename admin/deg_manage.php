<?php
	...
	if($_SESSION['userid']!="")
	{
		...
		$col = array();
		$col["title"] = "Deg Id"; // caption of column
		...
		$col["align"] = "center"; // this column is not editable
		$col["search"] = true; // this column is not searchable
		$col["editrules"] = array("integer"=>true,"limit"=>true,"required"=>true);
		# $col["formatter"] = "image"; // format as image -- if data is image url e.g. http://<domain>/test.jpg
		# $col["formatoptions"] = array("width"=>'20',"height"=>'30'); // image width / height etc
		$cols[] = $col;
		...
		$col["editrules"] = array("value"=>'A:Active;I:Inactive'); // and is required
		// $col["formatoptions"] = array("srcformat"=>'Y-m-d',"newformat"=>'d/m/Y'); // format as date, wont work for editing
		$cols[] = $col;
		
		$col = array();
		$col["title"] = "Entry by"; // caption of column
		$col["name"] = "c_entry_by"; // grid column name, must be exactly same as returned column-name from sql (tablefield or field-alias) 
		$col["width"] = "30";
		# $col["hidden"] = true; // hide column by default
		$cols[] = $col;
		...
		$col["name"] = "d_entry_date"; // grid column name, must be exactly same as returned column-name from sql (tablefield or field-alias) 
		$col["width"] = "30";
		# $col["hidden"] = true; // hide column by default
		$cols[] = $col;
		...
		$grid["sortname"] = "c_deg_id"; // by default sort grid by this field
		$grid["sortorder"] = "asc"; // ASC or DESC
		$grid["caption"] = "Degree Data"; // caption of grid
...
$g->select_command = "SELECT c_deg_id,c_deg_name,n_tot_sem,IF(c_status='A','Active','Inactive') as status FROM t_cms_degree";

// this db table will be used for add,edit,delete
$g->table = "t_cms_degree";
